...
const
  ...
  https = require('https'),
  fbSendAPI = require('./fbSendAPI.js');
...
app.post('/chatsim', function(req, res) {
        ...
        JSON.stringify({sender:senderID,'message':message});
        toBotStream.push('#'+JSON.stringify({sender:senderID,'message':message})+'\n');
      //  while (!msgsToSend[senderID]);
      setTimeout(function () {
        console.log("got resp from bot:",msgsToSend[senderID])
        res.send({'status':'OK','resp':msgsToSend[senderID]});
        msgsToSend[senderID]=0;
      },2000)
...
myConsole.on('line', (command) => {
    ...
    else if (command == 'tjson'){
      var someJson = JSON.stringify({sender:12,message:'ela mou'});
      toBotStream.push('#'+someJson+'\n');
    }
    myConsole.prompt();
}).on('close',function(){
    process.exit(0);
...
function processResp(resp)
{
  ...
  if (resp.SIM ){ //simulation
    console.log("SIM response from bot:",resp.txt);
    msgsToSend[resp.sessID]=resp.txt;
   }


   //OLD code following.. should change the chatSession[] Api.
/*
  if (resp.code =='0'){
    fbSendAPI.sendTextMessage(chatSession[resp.sessID],resp.txt);
  }
  else if (resp.code == '1'){
    //coffee order found
   ...
   fbSendAPI.sendTextMessage(chatSession[resp.sessID],validateOrderMsg);
  }
  else if (resp.code == '2'){
    console.log("About to send a plain list template to" +chatSession[resp.sessID])
    fbSendAPI.sendPlainListTemplate(chatSession[resp.sessID]);
  }
*/
}
